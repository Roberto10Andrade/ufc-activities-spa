...
function ActivitiesContent() {
  ...
  useEffect(() => {
    ...
    if (searchFromUrl) {
      const newFilter = { ...filter, search: searchFromUrl };
      setFilter(newFilter);
  ...
  const filteredActivities = activities.filter(activity => {
    if (appliedFilter.type !== 'ALL' && activity.type !== appliedFilter.type) return false;
    if (appliedFilter.status !== 'ALL' && activity.status !== appliedFilter.status) return false;
    if (appliedFilter.startDate) {
      const activityStartDate = format(new Date(activity.startDate), 'yyyy-MM-dd');
      if (activityStartDate < appliedFilter.startDate) return false;
    }
    if (appliedFilter.endDate) {
      const activityEndDate = format(new Date(activity.endDate), 'yyyy-MM-dd');
      if (activityEndDate > appliedFilter.endDate) return false;
    }
          ...
          <div className="border-b border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
            ...
            <div className="mt-4 flex items-center gap-3">
              <button
                ...
                className="inline-flex items-center justify-center px-6 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="inline-flex items-center justify-center px-6 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-lg shadow-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
